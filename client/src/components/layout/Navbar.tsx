    ...
    <header className="fixed top-4 md:top-6 left-0 right-0 z-50 flex justify-center px-2 sm:px-4 pointer-events-none w-full">
      ...
      >
        <div className="flex items-center justify-between w-full sm:w-auto gap-0 sm:gap-1 min-w-max px-0.5">
          ...
          <div className="flex items-center gap-0 sm:gap-1 mx-1 sm:mx-2 relative" onMouseLeave={() => setHoveredIdx(null)}>
            {navLinks.map((link, idx) => (
              <div 
                ...
                className="relative px-2.5 sm:px-5 py-1.5 sm:py-2.5 rounded-full cursor-pointer z-10 shrink-0"
              >
                {hoveredIdx === idx && (
                  <motion.div
                    layoutId="pill-hover"
                ...
                <span className={`relative z-10 text-[11px] sm:text-[14px] font-semibold transition-colors duration-200 whitespace-nowrap ${hoveredIdx === idx ? 'text-white' : 'text-gray-400'}`}>
                  {link.name}
                </span>
              </div>
            ))}
          ...
          <label className="theme-toggle cursor-pointer m-0 ml-1 sm:ml-3 scale-[0.6] sm:scale-75 origin-right shrink-0" aria-label="تبديل المظهر">
          ...
          <button 
            ...
            className="shrink-0 bg-white text-black px-4 sm:px-6 py-1.5 sm:py-2.5 rounded-full text-[11px] sm:text-sm font-bold shadow-lg hover:scale-105 transition-transform duration-200 ml-0 sm:ml-1 whitespace-nowrap"
          >
            اطلب الآن
          </button>
        </div>
