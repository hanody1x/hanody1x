  ...
  return (
    <header className="fixed top-4 md:top-6 left-0 right-0 z-50 flex justify-center px-4 sm:px-4 pointer-events-none w-full">
      ...
      <motion.nav 
        ...
        className="flex pointer-events-auto items-center p-2 md:p-1.5 rounded-full bg-[#1c1c1e]/95 border border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.4)] backdrop-blur-xl w-[95%] sm:w-max overflow-x-auto no-scrollbar max-w-full mx-auto"
        dir="rtl"
        style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}
      >
        <div className="flex items-center justify-between w-full sm:w-auto gap-0.5 sm:gap-1 min-w-max px-0.5">
          ...
          <div className="flex items-center gap-0 sm:gap-1 mx-1 sm:mx-2 relative" onMouseLeave={() => setHoveredIdx(null)}>
            {navLinks.map((link, idx) => (
              <div 
                ...
                className={`relative px-2 sm:px-5 py-1.5 sm:py-2.5 rounded-full cursor-pointer z-10 shrink-0 transition-all ${link.mobileHidden ? "hidden sm:block" : "block"}`}
              >
                {hoveredIdx === idx && (
                  <motion.div
                    layoutId="pill-hover"
                ...
                <span className={`relative z-10 text-[10.5px] sm:text-[14px] font-semibold transition-colors duration-200 whitespace-nowrap ${hoveredIdx === idx ? 'text-white' : 'text-gray-400'}`}>
                  {link.name}
                </span>
              </div>
            ))}
          ...
          <label className="theme-toggle cursor-pointer m-0 ml-1 sm:ml-3 transform scale-[0.65] sm:scale-[0.8] origin-right shrink-0" aria-label="تبديل المظهر">
